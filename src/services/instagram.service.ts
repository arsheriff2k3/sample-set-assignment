...
import { InstagramConfig, InstagramPost, InstagramResponse } from '../interfaces/instagram.interface';
import dotenv from 'dotenv';

dotenv.config();

/**
 * Service class for fetching Instagram data
 */
class InstagramService {
  ...
  private async fetchViaApi(): Promise<InstagramResponse> {
    try {
      ...
      // This is a simplified example of using the Instagram Graph API
      // In a real implementation, you would need to follow the proper authentication flow
  ...
  private async fetchViaPuppeteer(): Promise<InstagramResponse> {
    ...
    try {
      ...
      // Set a more realistic user agent to avoid being blocked
      ...
      // Navigate to the Instagram profile page with increased timeout
      try {
        ...
        logger.warn(`Navigation with networkidle2 failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      ...
      // Check if we need to handle cookie consent or login popups
      try {
      ...
      // Updated selectors for 2025 Instagram layout
      const postSelectors = [
        'article a, a[href*="/p/"]',
        'a[href*="/p/"]',
        'div[role="button"] a',
      ...
      // Get all post links using the working selector
      const postLinks = await page.$$eval(workingSelector, links => {
        return links.map(link => (link as HTMLAnchorElement).href).filter(href => href && href.includes('/p/'));
      });
      ...
      // Navigate directly to the first post URL instead of clicking
      ...
      // Extract the post data with updated selector options
      const postData = await page.evaluate(() => {
        // Updated caption selectors for 2025
        ...
        for (const selector of captionSelectors) {
          ...
          if (elements.length > 0) {
            // Usually the first or second span contains the caption
            for (let i = 0; i < elements.length; i++) {
              const text = elements[i].textContent;
              if (text && text.length > 5) { // Assume captions are at least a few characters
        ...
        // Updated image selectors for 2025
        ...
        for (const selector of imageSelectors) {
          const elements = document.querySelectorAll(selector);
          if (elements.length > 0) {
            // Usually the first image is the post image
      ...
      if (!postData.imageUrl) {
        // Try an alternative approach - get all images on the page
        ...
        // Filter for likely post images (usually larger images)
        const likelyPostImages = allImages.filter(img => 
          img.width > 300 && img.height > 300 && 
          img.src && img.src.includes('instagram')
        );
        
        if (likelyPostImages.length > 0) {
          logger.info(`Found ${likelyPostImages.length} potential post images`);
          postData.imageUrl = likelyPostImages[0].src !== null && likelyPostImages[0].src !== undefined ? likelyPostImages[0].src : '';
        } else {
  ...
  private async fetchViaCheerio(): Promise<InstagramResponse> {
    try {
      ...
      if (scriptTags.length === 0) {
        ...
        for (let i = 0; i < scriptTags.length; i++) {
          try {
            ...
            if (!jsonData) {
              const additionalDataMatch = scriptContent.match(/window\.__additionalDataLoaded\([^,]+,(.+)\);/);
            ...
            if (!jsonData) {
              const webSharedDataMatch = scriptContent.match(/__d\("InstagramWebSharedData",[^{]+({.+})\);/);
              if (webSharedDataMatch && webSharedDataMatch.length >= 2) {
                try {
                  ...
                  logger.warn(`Failed to parse InstagramWebSharedData: ${e instanceof Error ? e.message : 'Unknown error'}`);
                }
              }
            }
            ...
            if (jsonData) {
              try {
                ...
                const errorMessage = error instanceof Error ? error.message : 'Unknown error';
                logger.warn(`Failed to extract post data from script tag ${i+1}: ${errorMessage}`);
              }
            }
          } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      ...
      try {
        const postLinks = $('a[href*="/p/"]').map((_, el) => $(el).attr('href')).get();
        
        if (postLinks.length > 0) {
          const firstPostLink = postLinks[0];
          const postUrl = firstPostLink.startsWith('http') ? firstPostLink : `https://www.instagram.com${firstPostLink}`;
